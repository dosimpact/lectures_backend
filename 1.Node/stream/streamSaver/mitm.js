...
function onMessage(event) {
  ...
  // It's important to have a messageChannel, don't want to interfere with other simultaneous downloads
  ...
  // the default public service worker for StreamSaver is shared among others. so all download links needs to be prefixed to avoid any other conflict
  data.origin = origin;

  // if we ever (in some feature versoin of streamsaver) would like to redirect back to the page of who initiated a http request
  ...
  //The service worker can  then use the transferred port to reply via postMessage(), which will in turn trigger the onmessage handler on messageChannel.port1.
