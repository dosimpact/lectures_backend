const streamSaverCore = (() => {
  ...
  const ponyfill = globalThis.WebStreamsPolyfill || {};
  const isSecureContext = globalThis.isSecureContext;
  // TODO: Must come up with a real detection test (#69)
  let useBlobFallback =
    /constructor/i.test(globalThis.HTMLElement) ||
    !!globalThis.safari ||
    !!globalThis.WebKitPoint;
  ...
  const streamSaver = {
    createWriteStream,
    WritableStream: globalThis.WritableStream || ponyfill.WritableStream,
    supported: true,
  ...
  function makePopup(src) {
    const options =
      "width=1,height=1,visible=none,left=10000,top=10000,toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no";
    const delegate = document.createDocumentFragment();
    const popup = {
      frame: globalThis.open(src, "popup", options),
      loaded: false,
      isIframe: false,
      isPopup: true,
      remove() {
...
export default streamSaverCore;
